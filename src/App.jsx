...
function MainApp(props) {
    ...
    const [channelModalMode, setChannelModalMode] = useState('create'); // 'create' or 'edit'
    ...
    const handleUpdateChannel = (id, type, currentName) => {
        if (userRole !== 'admin' && !isCeo) return;
    ...
    const confirmCreateChannel = async () => {
        if (!newChannelName || !newChannelName.trim()) {
            showToast('error', 'Kanal adı boş olamaz.');
            return;
        }
        ...
        try {
            if (channelModalMode === 'create') {
                ...
                const channelDocRef = doc(db, 'artifacts', APP_ID, 'public', 'data', 'channels', editingChannelId);
                await setDoc(channelDocRef, { name: newChannelName.trim() }, { merge: true });
            ...
            console.error("Kanal işlemi hatası:", err);
            showToast('error', `Kanal ${channelModalMode === 'create' ? 'oluşturulamadı' : 'güncellenemedi'}.`);
        }
    };
    ...
    return (
      ...
      `}>
            ...
            {/* Premium Kanal Oluşturma Modalı */}
            {isCreateChannelModalOpen && (
                <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 animate-fade-in">
                    <div className="absolute inset-0 bg-black/80 backdrop-blur-md" onClick={() => setIsCreateChannelModalOpen(false)}></div>
                    <div className="bg-[#0a0a0c] border border-white/5 rounded-[32px] w-full max-w-[400px] shadow-2xl overflow-hidden relative z-10 animate-scale-in origin-center ring-1 ring-white/10 flex flex-col transition-all duration-300">
                        {/* Header */}
                        <div className={`h-32 bg-gradient-to-br ${newChannelType === 'text' ? 'from-indigo-600/40 via-purple-600/20' : 'from-emerald-600/40 via-teal-600/20'} to-[#0a0a0c] relative p-8 flex items-center justify-between shrink-0`}>
                            <div className="absolute inset-0 bg-[length:20px_20px] bg-center [background-image:radial-gradient(ellipse_at_center,rgba(255,255,255,0.05)_1px,transparent_1px)] opacity-50"></div>
                            <div className="relative flex items-center space-x-4">
                                <div className={`p-3 rounded-2xl ring-1 shadow-lg ${newChannelType === 'text' ? 'bg-indigo-500/20 text-indigo-400 ring-indigo-500/30' : 'bg-emerald-500/20 text-emerald-400 ring-emerald-500/30'}`}>
                                    {newChannelType === 'text' ? <Hash size={24} /> : <Volume2 size={24} />}
                                </div>
                                <div>
                                    <h2 className="text-xl font-black text-white tracking-tight uppercase">{channelModalMode === 'create' ? 'Kanal Oluştur' : 'Kanalı Düzenle'}</h2>
                                    <p className={`text-[10px] font-bold uppercase tracking-widest ${newChannelType === 'text' ? 'text-indigo-300/80' : 'text-emerald-300/80'}`}>
                                        {newChannelType === 'text' ? 'Yazı Kanalı' : 'Ses Odası'}
                                    </p>
                                </div>
                            </div>
                            <button onClick={() => setIsCreateChannelModalOpen(false)} className="relative p-2 bg-black/40 hover:bg-white/10 text-slate-300 rounded-full backdrop-blur-md transition-colors border border-white/10 hover:text-white">
                                <X size={18} />
                            </button>
                        </div>
                        ...
                        <div className="p-8 space-y-6">
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest px-1">Kanal Adı</label>
                                <div className="relative group">
                                    <div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-indigo-400 transition-colors">
                                        {newChannelType === 'text' ? <Hash size={16} /> : <Volume2 size={16} />}
                                    </div>
                                    <input
                                        autoFocus
                                        type="text"
                                        ...
                                        placeholder={newChannelType === 'text' ? "yazı-kanalı-adı" : "Ses Odası Adı"}
                                        className="w-full bg-white/[0.03] border border-white/5 rounded-2xl pl-12 pr-4 py-4 text-white text-base focus:outline-none focus:border-indigo-500/30 focus:bg-white/[0.06] transition-all font-medium placeholder-slate-600"
                                    />
                                </div>
                            </div>
                            ...
                            <div className="flex space-x-3 pt-2">
                                <button
                                    ...
                                    className="flex-1 py-4 bg-white/5 hover:bg-white/10 text-slate-400 rounded-2xl font-bold text-sm transition-all"
                                >
                                    İptal
                                </button>
                                <button
                                    ...
                                    className={`flex-[2] py-4 rounded-2xl font-black text-sm uppercase tracking-widest transition-all shadow-lg active:scale-95 disabled:opacity-50 disabled:active:scale-100 ${newChannelType === 'text' ? 'bg-indigo-500 hover:bg-indigo-400 text-white shadow-indigo-500/20' : 'bg-emerald-500 hover:bg-emerald-400 text-white shadow-emerald-500/20'}`}
                                >
                                    {channelModalMode === 'create' ? 'Oluştur' : 'Güncelle'}
                                </button>
                            </div>
                        </div>
                    </div>
